...
bangaloreCalls = [call for call in calls if call[0].startswith("(080)")]

bangaloreDialed = [call[1] for call in bangaloreCalls]

called_areas = []

for call in bangaloreDialed:
    if re.search("\(\w+\)", call):
...
total_calls = len(bangaloreCalls)

bangalore_bangalore = len([call for call in bangaloreCalls if call[
    1].startswith("(080)")])
